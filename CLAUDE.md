...
- Paramètres du foyer (singleton, GET + PATCH) : `/api/v1/referentiels/parametres-budget/` → `{ jour_debut_mois_comptable }` (1–28, défaut 1). Pilote le découpage en mois comptables. **Le PATCH remappe automatiquement tout l'historique des flux** si le jour change (`ParametresBudgetView.perform_update` → `flux/services/recalcul_mois.py::recalculer_mois_flux()`) : l'UI n'a pas à lancer la commande à la main.
...
- Analyse rétrospective (Phase 13) : `/api/v1/analytics/analyse/?nb_mois=6` (fenêtre glissante 3/6/12/24 mois comptables, lecture seule, **fiabilité RÉELLE**, transferts + ajustements exclus). 4 blocs : `tendances` (série mensuelle dépenses/revenus/épargne/taux + `totaux_periode`, `moyennes_mensuelles`, `comparaison_periode_precedente` neutre N vs N-1), `titulaires` (ventilation par propriétaire de compte : `par_titulaire` = {nom, est_commun, depenses, revenus, epargne_nette, part_depenses_pct} + `commun_vs_perso` ; **comptes communs = bucket « Commun » distinct**, jamais rattaché à une personne — arbitrage foyer), `categories` (par catégorie majeure : `total_periode`, `moyenne_mensuelle`, `part_pct`, `serie` mensuelle ; mineures agrégées sous parent), `rythme` (`par_jour_semaine` 1=lundi…7=dimanche + `libelles_recurrents` ≥2 occurrences). Service `analytics/services/analyse.py::calculer_analyse(nb_mois, aujourd_hui=None)`, aucune migration, **aucune projection** (le prévisionnel reste séparé). **14 tests**.
...
| Plus | `/plus` | ✅ menu mobile (accès **Analyse**/Prévisionnel/Comptes/Abonnements/Patrimoine/Catégories/**Paramètres**) + toggle thème |
...
- **Backend** : service `analytics/services/analyse.py::calculer_analyse(nb_mois=6, aujourd_hui=None)` (miroir de `projection.py`/`trajectoire.py`), `AnalyseView` + `AnalyseSerializer`, route `/api/v1/analytics/analyse/`. Fenêtre bornée par `core/services/periode.py` (mois comptable) ; filtre `flux.mois` auto-correct. Blocs `tendances` / `titulaires` / `categories` / `rythme` (détail dans « Endpoints clés »). `ExtractIsoWeekDay` pour le jour de semaine ; libellés récurrents regroupés par libellé normalisé (minuscule + espaces) en Python. **14 tests** (`AnalyseServiceTest` 12 + `AnalyseAPITest` 2 → suite à 336 OK).
- **Frontend** : `AnalysePage` + `useAnalyse` (préfixe `'analytics'` → invalidé par les mutations de flux). `BarChart` étendu (prop `stacked`, légende auto si >1 série). Lien d'accès dans l'en-tête du Dashboard (`ChartColumn`) + Sidebar + Plus. `DEFINITIONS` : `analyse_tendances`, `analyse_comparaison`, `analyse_titulaires`, `analyse_commun_perso`, `analyse_categories`, `analyse_rythme_jour`, `analyse_recurrents`.
...
**Incréments futurs — ordre convenu avec l'utilisateur** : (2) **saisonnalité** — annuelle N vs même mois année-1, heatmap multi-mois — PROCHAIN ; puis (3) **axe Épargne** — taux dans le temps + flux réels vers comptes d'épargne via `Transfert` + cumul (l'angle mort actuel). À cadrer en session avant de coder chacun.
