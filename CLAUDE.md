...
- **Backend** : service `analytics/services/analyse.py::calculer_analyse(nb_mois=6, aujourd_hui=None)` (miroir de `projection.py`/`trajectoire.py`), `AnalyseView` + `AnalyseSerializer`, route `/api/v1/analytics/analyse/`. Fenêtre bornée par `core/services/periode.py` (mois comptable) ; filtre `flux.mois` auto-correct. Blocs `tendances` / `titulaires` / `categories` / `rythme` / `saisonnalite` (détail dans « Endpoints clés »). `ExtractIsoWeekDay` pour le jour de semaine ; libellés récurrents regroupés par libellé normalisé (minuscule + espaces) en Python. **18 tests** (`AnalyseServiceTest` 16 + `AnalyseAPITest` 2 → suite à 340 OK).
- **Frontend** : `AnalysePage` + `useAnalyse` (préfixe `'analytics'` → invalidé par les mutations de flux). `BarChart` étendu (prop `stacked`, légende auto si >1 série). Lien d'accès dans l'en-tête du Dashboard (`ChartColumn`) + Sidebar + Plus. `DEFINITIONS` : `analyse_tendances`, `analyse_comparaison`, `analyse_titulaires`, `analyse_commun_perso`, `analyse_categories`, `analyse_rythme_jour`, `analyse_recurrents`, `analyse_saisonnalite`.
...
**Incrément futur restant — convenu avec l'utilisateur** : (3) **axe Épargne** — taux d'épargne dans le temps + flux réels vers comptes d'épargne via `Transfert` + cumul (l'angle mort actuel : l'épargne réelle passe par des transferts, exclus partout, donc visualisée nulle part). À cadrer en session avant de coder.
