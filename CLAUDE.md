...
**Charts** (`src/components/charts/`) : `chartSetup.js` (palette `CAT_PALETTE` 12 couleurs dans DashboardPage), `LineChart`, `BarChart`, `DoughnutChart`, `DepensesCategories` (donut + légende dépliable + drill-down via props `mois`/`compteId`), `HeatmapDepenses` (calendrier des dépenses du mois, props `data`=`depenses_par_jour`/`mois`). Le drill-down ouvre `FluxCategorieModal` (`src/components/flux/`) qui réutilise l'endpoint `/flux/` (filtres `categorie`+`mois`+`est_transfert`, +`compte` si scopé) — aucun nouvel endpoint.
