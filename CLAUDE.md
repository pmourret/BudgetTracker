...
- Rapprochement bancaire (Phase 14-A) : `POST /api/v1/imports/` (**multipart** `compte` + `banque` + `fichier` → parse+dédup+rapprochement, renvoie `{lot, nb_doublons, erreurs_parsing}` ; 400 sur format invalide, banque non supportée, ou **fichier multi-comptes** avec la liste des `accountNum`), `GET /api/v1/imports/{id}/rapport/` (rapport détaillé : `controle_solde`, `lignes:[{statut, flux_detail, candidats:[FluxResume]}]`, `flux_sans_ligne:[{...FluxResume, motif}]`), `POST /api/v1/imports/{id}/relancer/`, `DELETE /api/v1/imports/{id}/` (soft delete cascadé). Lignes : `GET /api/v1/imports-lignes/{id}/candidats/`, `POST /api/v1/imports-lignes/{id}/valider/` (body `{"flux_id": ...}`), `POST /api/v1/imports-lignes/{id}/rejeter/`.
...
### 🟢 Phase 14 — Rapprochement bancaire (app `imports`) — 14-A LIVRÉE (back + front, juillet 2026)
...
**⏳ 14-B (à venir)** : créer les flux manquants en 1 clic depuis un écart + **pointage durable** via `Flux.reference_externe`. **Incrément possible** : exposer le `controle_solde` par compte hors import (widget). Ne PAS toucher aux flux depuis le rapprochement sans passer par 14-B (règle lecture seule 14-A).
...
- **Rapprochement bancaire (Phase 14-A) : lecture seule vis-à-vis des flux** — le moteur `imports/services/rapprochement.py` ne crée/modifie JAMAIS un flux ; il n'écrit que l'état de rapprochement de la `LigneBancaire` (`statut`/`flux`). La création des flux manquants et le pointage durable (`Flux.reference_externe`) sont réservés à 14-B. Ne pas court-circuiter cette règle.
- **`accountbalance` BoursoBank = instantané journalier, pas par opération** : les lignes d'un même jour partagent le même solde (constaté sur l'échantillon foyer). Conséquences : (1) **pas d'unicité dure** sur `LigneBancaire.hash_dedup` (deux opérations identiques le même jour se distingueraient mal → l'anti-doublon se fait par **comptage d'occurrences** dans `filtrer_doublons`, jamais par contrainte DB) ; (2) le `controle_solde` se base sur la ligne la plus récente et compare **au centime** (pas de seuil arbitraire — règle 1).
...
- **Upload multipart : curl sur Windows n'aime pas `-F "fichier=@x.csv;type=text/csv"`** (renvoie `HTTP 000`) → utiliser `-F "fichier=@x.csv"` sans le `;type=`. Côté front, `axios` retire automatiquement le `Content-Type: application/json` par défaut de l'instance quand le corps est un `FormData` et laisse le navigateur poser le boundary — ne PAS forcer un `Content-Type` multipart à la main (boundary manquant). Le hook `useUploadImport` construit le `FormData` (`compte`/`banque`/`fichier`).
