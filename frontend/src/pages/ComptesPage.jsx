...
import { useResourceList, useDeleteResource, useUpdateResource, useResourceAction } from '../hooks/useResource'
import { formatEuro } from '../utils/format'
import Card from '../components/ui/Card'
import Button from '../components/ui/Button'
import { Loading, ErrorState, EmptyState } from '../components/ui/States'
import IconBadge from '../components/ui/IconBadge'
import { Landmark, Pencil, Trash2, RefreshCw } from 'lucide-react'
...
export default function ComptesPage() {
  ...
  if (isLoading) return <Loading message="Chargement des comptes..." />
  if (isError) {
      ...
      {comptes.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            <MetricCard label="Solde total théorique" value={formatEuro(totalTheorique)} />
            <MetricCard label="Solde réel cumulé" value={formatEuro(totalReel)} />
            <MetricCard
              label="Écart total"
              value={formatEuro(ecartTotal)}
              valueClass={ecartTotal === 0 ? 'text-teal-600' : 'text-amber-600'}
            />
          </div>
...
function CompteCard({ compte, onEdit }) {
  ...
  const reconcilierAction = useResourceAction('comptes')
  const [deleteError, setDeleteError] = useState(null)
  const [reconcilierMsg, setReconcilierMsg] = useState(null)
  ...
  const handleReconcilier = () => {
    const ecartLabel = ecart > 0 ? `+${formatEuro(ecart)}` : formatEuro(ecart)
    if (!window.confirm(
      `Un flux d'ajustement de ${ecartLabel} va être créé pour équilibrer le solde.\n\nContinuer ?`
    )) return
    setReconcilierMsg(null)
    reconcilierAction.mutate(
      { id: compte.id, action: 'reconcilier' },
      {
        onSuccess: (data) => setReconcilierMsg(data.detail),
        onError: (err) => setReconcilierMsg(err.response?.data?.detail || 'Erreur lors de la réconciliation.'),
      }
    )
  }
  ...
  return (
    <Card>
      ...
      <div className="text-[11px] text-content-2">Solde théorique</div>

      <div className="flex justify-between mt-4 pt-3.5 border-t border-border-app">
        <div>
          <div className="text-[11px] text-content-2">Solde réel</div>
          <div className="text-sm font-medium text-content">{formatEuro(compte.solde_reel)}</div>
        </div>
        <div className="text-right">
          <div className="text-[11px] text-content-2">Écart</div>
          <div className={`text-sm font-medium ${ecart === 0 ? 'text-teal-600' : 'text-amber-600'}`}>
            {formatEuro(ecart)}
          </div>
        </div>
      </div>
      ...
      {ecart !== 0 && (
        <button
          ...
          disabled={reconcilierAction.isPending}
          className="mt-3 w-full flex items-center justify-center gap-1.5 text-xs font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 hover:bg-amber-100 cursor-pointer disabled:opacity-50 transition-colors"
        >
          <RefreshCw size={12} />
          {reconcilierAction.isPending ? 'Réconciliation...' : 'Réconcilier le solde'}
        </button>
      )}

      {reconcilierMsg && (
        <div className="mt-2 text-xs text-teal-700 bg-teal-50 border border-teal-200 rounded-lg px-3 py-2">
          {reconcilierMsg}
        </div>
      )}
