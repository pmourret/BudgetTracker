...
3. `Mois` (libellé = 1er du mois) est calculé automatiquement depuis `Date_Flux` selon le **mois comptable** du foyer : paramètre administrable `jour_debut_mois_comptable` (singleton `ParametresBudget`, défaut 1 = calendaire). Point unique de vérité : `core/services/periode.py`. Convention : une période démarrant en fin de mois porte le libellé du mois qu'elle **finance** (jour 25 → 25 juin→24 juillet = « juillet »). Borné à 28.
...
**Endpoints clés** : CRUD sur toutes les ressources (y compris `/categories/`, `/budget-templates/`), référentiels en lecture seule sauf Titulaire et Établissement, `/referentiels/parametres-budget/` (singleton GET + PATCH), `/patrimoine/total/`, `/patrimoine/historique/`, `/patrimoine/verifier-rappels/`, `/analytics/dashboard/?nb_mois=6`, `/analytics/previsionnel/?nb_mois=6`, + actions custom (`verifier-divergence`, `valoriser`, `acquitter`, `acquitter-tout`, `/budget-templates/reconduire/`).

**Mois comptable (paramétrable)** : singleton `ParametresBudget.jour_debut_mois_comptable` (1–28, défaut 1, créé par `seed_referentiels`). `core/services/periode.py` (`mois_comptable`, `bornes_mois_comptable`, `mois_comptable_courant`) est le point unique de vérité — `Flux._calculer_mois` et tous les services analytics (dashboard, compte_dashboard, projection, trajectoire) en dépendent ; filtrer par `flux.mois` est auto-correct, filtrer par plage de dates exige `bornes_mois_comptable`. Avec jour=1, retombe sur le calendaire (rétro-compat). `ParametresBudgetView.perform_update` déclenche `flux/services/recalcul_mois.py::recalculer_mois_flux()` (aussi exposé en commande `recalculer_mois`) → remap transparent au changement de jour. Suite à **288 tests OK**.
...
| Paramètres | `ParametresPage` (route `/parametres`, Sidebar icône `Settings` + menu Plus). Carte « Mois comptable » : `Select` jour de bascule (1–28), exemple de période dynamique, avertissement de recalcul si modifié, bouton Enregistrer + feedback, `Tooltip` `DEFINITIONS.mois_comptable`. Hook `useParametres` (GET + PATCH `parametres-budget`, invalide flux/comptes/budgets/alertes/analytics). Le remap des flux est fait backend au PATCH. Page extensible pour de futurs réglages. |

**Dashboard** : carte « Dépenses par catégorie » avec `DoughnutChart` (donut) par majeure + légende expandable (clic → affiche les mineures avec montant et %), drill-down `FluxCategorieModal`. Palette 12 couleurs. Carte « Calendrier des dépenses » (`HeatmapDepenses`) : grille calendaire **compacte** (hauteur de cellule fixe, pas `aspect-square`), intensité rouge par jour **plafonnée au 90e centile** (`calculerPlafond` — un gros flux isolé n'écrase plus l'échelle), jours à venir atténués, drill-down au clic/Entrée sur un jour → `FluxJourModal` (dépenses du jour). Sélecteur de période extrait en composant partagé `PeriodSelector` (réutilisé par Prévisionnel).
...
- **Heatmap dépenses : échelle plafonnée au 90e centile, cellules à hauteur fixe** : `HeatmapDepenses` colore chaque jour selon `total / plafond` (`plafond = 90e centile des jours dépensés`, `calculerPlafond`), jamais `total / max` (un gros flux isolé écrasait toute l'échelle → heatmap monochrome). Plancher d'opacité `0.4` + texte `text-content-3` sur les niveaux faibles (sinon chiffre illisible en thème clair). Cellules `h-12 sm:h-14`, jamais `aspect-square` (sur desktop le calendrier remplissait tout l'écran, à l'opposé de l'esprit dashboard). Drill-down jour via `FluxJourModal` qui réutilise `/flux/` (filtres `date_min`=`date_max`+`est_transfert`, dépenses seules) — aucun nouvel endpoint.
- **`useResourceList(resource, params, options)`** : le 3e argument `options` est étalé dans `useQuery`. Sert notamment à passer `{ enabled: isOpen }` pour qu'un modal de drill-down (`FluxJourModal`) ne déclenche aucune requête tant qu'il est fermé.
